...
if(isset($_POST['subBtn'])){
    ...
    if(count($errors)==0 and $img_uploadOk==1 and $sign_uploadOK==1){
        ...
        $query="INSERT INTO nurses (regId,fname,lname,email,address,city,district,nation,pincode,phone,gender,age,salary,employee_type,photo,signature) VALUES ('$regId','$fname','$lname','$email','$address','$city','$district','$nation','$pincode','$phone','$gender','$age','$salary','$emp_type','$img_target_file','$sign_target_file')";
...
if(isset($_POST['deductBtn'])){
    ...
    $real_hra = floatval(($hra*$nurseSal)/100);
    ...
    $dquery="INSERT INTO nurse_salary_infos (nurse_id,HRA,PF,ESIC,Advance,Bonus) VALUES ('$id','$real_hra','$pf','$real_esic','$advpay','$real_bonus')";
...
if(isset($_POST['splAlowBtn'])){
    $special_allowance=(int)mysqli_real_escape_string($db,$_POST['specialallow']);
    $nId=mysqli_real_escape_string($db,$_POST['nurse_Id']);

    echo $nId;
    $special_query = "UPDATE nurse_salary_infos SET special_allowance='$special_allowance' WHERE nurse_id='$nId'";
    if(mysqli_query($db,$special_query)){
        echo "Done";
    }else{
        echo mysqli_error($db);
    }
            ...
            <div class="card-body border">
                <div class="row">
                    <div class="col-md-4 d-flex justify-content-start mb-2">
                        <button class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#salaryModal" onclick="setInfoId(<?php echo $row['id'];?>,<?php echo $row['salary']; ?>)">Salary Info</button>
                                    ...
                                    <div class="modal-body">
                                        <form action="" method="post">
                                                ...
                                                <div class="form-group ">
                                                    ...
                                                    <input type="number" name="hra"  class="form-control" min="0" required placeholder="Value will be in %">
                                                </div>
                                                <div class="form-group ">
                                                        ...
                                                        <?php
                                                            if($row['employee_type']=='contactual'){
                                                                ...
                                                                ?>
                                                                    ...
                                                                    <div class="col-4  ">
                                                                        <a class="btn btn-warning btn-sm" id="autoEsic" onclick="hideEsic()">Auto</a>
                                                                    </div>
                                                                    <div class="col-4 ">
                                                                        <a class="btn btn-success btn-sm " id="manEsic" onclick="showEsic()">Manual</a>
                                                                    </div>
                                                                <?php
                                                            }
                                                        ?>
                                                ...
                                                <input type="hidden" name="nurseId" id="infoNurseId" value="">
                                                <input type="hidden" name="nurseSal" id="infoNurseSalary" value="">
                    ...
                    <div class="col-6">
                        <button class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#specialModal" onclick="setNurseId(<?php echo $row['id'] ?>)">Special Allowance</button>

                        <div class="modal fade" id="specialModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Special Allowance per Month</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <form action="" method="post">
                                            <div class="form-group ">
                                                <label for="spall" class="font-weight-bold">Amount<span class="star">*</span></label>
                                                <input type="number" name="specialallow" id="spall"  class="form-control" min="0" required placeholder="Amount in &#8377;">
                                            </div>

                                            <div class="col-12 d-flex justify-content-end">
                                                <button type="button" class="btn btn-secondary mr-2" data-dismiss="modal">Close</button>
                                                <button type="submit" name="splAlowBtn" class="btn btn-primary">Save</button>
                                            </div>
                                            <input type="hidden" name="nurse_Id" id="nurse_id_input" value="">
                                        </form>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-md-2 d-flex justify-content-end mb-2 align-items-center">
